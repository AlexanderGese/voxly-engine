...
// xp curve: each level costs a bit more than the last. cheap quadratic-ish
// ramp, capped so the per-level cost doesnt explode. cumulative cost to reach
// level L is the running sum of per-level costs.
...
void hud2_xpbar_set(hud2_xpbar *xp, int total_xp) {
    ...
    // walk levels until the next one would cost more than we have left.
    int level = 0;
...
void hud2_xpbar_update(hud2_xpbar *xp, float dt) {
    ...
    // on a level-up the target frac drops (we crossed a boundary); let the
    // display fill to full first then snap so it reads as "completed the bar".
