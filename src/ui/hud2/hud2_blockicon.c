...
void hud2_block_icon(hud2_batch *b, block_id id,
    ...
    float hw = sw * 0.5f;
    float hh = sh * 0.5f;
    float th = hh * 0.5f;          // vertical extent of the top diamond half

    // key points
    float top_x  = cx,        top_y  = cy - hh;          // peak
    float left_x = cx - hw,   left_y = cy - hh + th;     // upper-left
    float right_x= cx + hw,   right_y= cy - hh + th;     // upper-right
    float mid_x  = cx,        mid_y  = cy - hh + th * 2; // front-mid (top center)
    float bl_x   = cx - hw,   bl_y   = cy + hh - th;     // lower-left
    float br_x   = cx + hw,   br_y   = cy + hh - th;     // lower-right
    float bot_x  = cx,        bot_y  = cy + hh;          // bottom point
    ...
    // right face: darkest, gives it some depth
