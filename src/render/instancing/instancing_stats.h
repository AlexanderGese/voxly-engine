...
// fold a finished batch into the running totals. survivors is the post-cull
// count; base_tris is triangles in one copy of the base mesh.
void instancing_stats_add_batch(instancing_stats *s, int survivors,
                                int base_tris, int draw_calls);

// crude human-readable dump into `buf`. returns chars written (sans null).
