...
// x and (x^y), which is the closed form of the recursion. no allocation.
...
float volumetric_dither_at(const volumetric_dither *d, int px, int py) {
    ...
    // wrap into the tile. C's % can go negative, so mask back positive.
    int x = px % d->dim; if (x < 0) x += d->dim;
    int y = py % d->dim; if (y < 0) y += d->dim;
