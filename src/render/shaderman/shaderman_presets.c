...
// the stock table. order doesnt matter, names must be unique. geom is NULL for
// all of these — none of the base passes use a geometry stage (yet).
typedef struct {
    const char *name;
    const char *vert;
    const char *frag;
...
// write a handle into the right field of the presets struct via its offset.
