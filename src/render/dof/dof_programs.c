...
// thin wrapper around glGetUniformLocation that tolerates a 0 program (returns
// -1) so the load path doesnt have to guard every lookup.
...
void dof_programs_unload(dof_programs *p) {
    if (p->coc)       gl_delete_shader(p->coc);
    if (p->gather)    gl_delete_shader(p->gather);
    if (p->composite) gl_delete_shader(p->composite);
    p->coc = p->gather = p->composite = 0;
    p->ok = 0;
}

void dof_programs_upload_kernel(const dof_programs *p, const dof_kernel *k) {
    if (!p->gather || p->u_gather.taps < 0) return;

    // pack to a tight float pair array and upload as a vec2[]. doing it here
    // means the gather pass never re-uploads static data per frame.
