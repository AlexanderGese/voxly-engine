...
// at the horizon doesnt try to gather across the whole screen.
#define DOFX_MAX_COC_TEXELS     16.0f

// below this coc (in texels) we treat the fragment as in focus and skip the
// gather entirely. keeps the focal plane crisp and saves taps.
#define DOFX_FOCUS_EPSILON      0.5f

// the near field has to bleed *over* sharp geometry, so we dilate the near
// coc by this many half-res texels before compositing. without it the near
// blur gets a hard sharp edge where it meets the in-focus midground.
#define DOFX_NEAR_DILATE        2

// final blend amount. 1 = full dof, 0 = passthrough. lets the options screen
// dial the whole effect down without retuning the lens.
#define DOFX_DEFAULT_STRENGTH   1.0f

// texture unit assignments. the gather pass binds color + coc; the composite
// pass binds the sharp color plus the near/far gather results.
...
// shader paths. live next to the rest under shaders/. the .frag's are
// optional at runtime — if theyre missing we disable the gpu path and the
// cpu reference (coc/blend) still works for tests.
