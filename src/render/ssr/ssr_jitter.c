...
// cheap, no texture, tiles well enough that the blur erases the seam.
static float fract(float x) { return x - floorf(x); }

float ssrx_jitter_ign(const ssrx_jitter *j, int px, int py) {
    float x = (float)px;
    float y = (float)py;
    // shift the lattice each frame when animating so taa-style accumulation
    // sees a fresh pattern. the 5.588 step is arbitrary-but-irrational-ish.
    if (j->frame != 0.0f) {
        x += j->frame * 5.588238f;
        y += j->frame * 5.588238f;
    }
...
vec2 ssrx_jitter_dir(const ssrx_jitter *j, int px, int py) {
    // decorrelate the second axis by offsetting the lattice; gives a usable 2d
    // rotation without a second noise function.
    float a = ssrx_jitter_ign(j, px, py);
    float b = ssrx_jitter_ign(j, px + 37, py + 17);
    // map to a unit-ish vector. not normalized on purpose — magnitude jitter is
    // fine for a blur disk rotation.
