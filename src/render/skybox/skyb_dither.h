...
// target bit depth (usually 8); the nudge is scaled to ~1 LSB at that depth.
skyb_rgb skyb_dither_color(skyb_rgb c, int cell, int bits);

// quantize a color to `bits` per channel, applying dither first. this is what
// the dome bake calls so the uploaded vertex colors are already broken up.
