...
// is the region containing `pos` below SPAWNX_REGION_CAP? an untracked region
// (table full) is treated as having room, because the global caps still apply.
int  spawnx_region_has_room(const spawnx_region_map *m, vec3 pos);

// fold a just-committed spawn into the region tally so a single cycle that adds
// several mobs to one region still respects the cap mid-loop.
