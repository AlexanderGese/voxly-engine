...
// "can i shortcut straight to the target" check wants the same test.
int nav_funnel_clear(world *w, vec3 a, vec3 b, float width);

// smooth `in` into `out` in place-safe fashion (in and out may alias). keeps
// the first and last points, drops any interior point both neighbours can see
// past. returns the resulting waypoint count.
