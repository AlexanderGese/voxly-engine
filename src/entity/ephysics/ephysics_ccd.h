...
// how many sub-steps a move of this delta would need. exposed for stats/debug.
int ephysics_ccd_substeps(vec3 delta);

// resolve `delta` against the world with automatic sub-stepping. re-gathers the
// broadphase per sub-step so candidates stay valid. records contacts into `man`
// (may be NULL). `iters` is the per-sweep slide-pass count handed to the
// resolver. returns the unconsumed remainder of delta.
