...
// static description of a biome. surface stacking + flavor knobs.
typedef struct {
    const char *name;
...
// per-column work context. the biome surface fns read/fill this.
// y axis: 0..height-1 are solid candidates, we just report ids.
typedef struct {
...
// signature every biome module exposes: given a column and a y, what block?
// returns BLOCK_AIR above the surface. pure, no side effects.
