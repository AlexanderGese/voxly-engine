...
// chunk so it cant tell what's diggable — that filter is the caller's problem,
// same split rivers used for its edit list.
//
// the returned array is a darray (darr_len / darr_free). it can get big for a
// fat ravine, so free it when youre done staring at it.

// build the edit list for the resolved field. strata supplies the wall bands.
// returns a darray of ravine_edit (caller frees with darr_free), or NULL if the
// field is empty. *out_count gets the length for convenience.
ravine_edit *ravine_emit_build(const ravine_field *f,
                               const ravine_strata *strata,
                               const ravine_params *p, size_t *out_count);

// count how many edits the field would produce without allocating them. handy
// for the overlap checker that just wants a cheap "how invasive is this".
