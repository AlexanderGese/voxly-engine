...
float heightmap_blend_sample(const heightmap_patch *patch, int wx, int wz) {
    ...
    // clamp the corner indices into the lattice, fold the fraction so the edge
    // reads flat instead of wrapping
    if (i0 < 0)            { i0 = 0; tx = 0.0f; }
    if (i0 > cells - 2)    { i0 = cells - 2; tx = 1.0f; }
    if (j0 < 0)            { j0 = 0; tz = 0.0f; }
    if (j0 > cells - 2)    { j0 = cells - 2; tz = 1.0f; }
    int i1 = i0 + 1;
    int j1 = j0 + 1;
...
// only the steepness gets smoothed; the actual surface y stays exact because
