...
// block_face_tile), `corner` is 0..3 around that face. returns blended 0..1.
// this is what the mesher calls four times per face.
float lp_sample_vertex(world *w, int wx, int wy, int wz,
                       int face, int corner, float daylight);

// raw separated channels for a face center, if a shader wants block/sky split
// (e.g. to tint torch light warm and skylight cool). values are 0..15.
