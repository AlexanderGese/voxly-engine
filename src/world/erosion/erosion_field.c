...
vec2 erosion_sample_gradient(const erosion_field *f, vec2 p) {
    ...
    // analytic partials of the bilinear patch. points uphill.
...
float erosion_sample_hardness(const erosion_field *f, vec2 p) {
    ...
    float h00 = f->hardness[erosion_idx(x0,     z0)];
    float h10 = f->hardness[erosion_idx(x0 + 1, z0)];
    float h01 = f->hardness[erosion_idx(x0,     z0 + 1)];
    float h11 = f->hardness[erosion_idx(x0 + 1, z0 + 1)];
...
float erosion_cell_relief(const erosion_field *f, int x, int z) {
    float here = erosion_height_at(f, x, z);
    float worst = 0.0f;
    for (int dz = -1; dz <= 1; dz++) {
        for (int dx = -1; dx <= 1; dx++) {
            if (!dx && !dz) continue;
            float d = fabsf(here - erosion_height_at(f, x + dx, z + dz));
            if (d > worst) worst = d;
        }
    }
    return worst;
