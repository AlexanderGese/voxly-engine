...
// remove the source at a position. returns true if one was removed.
bool voxl_fluid_source_remove(voxl_fluid_source_set *s, int x, int y, int z);

// is there an active source here?
bool voxl_fluid_source_at(const voxl_fluid_source_set *s, int x, int y, int z);

// stamp every active source back to a full cell on the grid. call before
// each spread step.
void voxl_fluid_sources_apply(const voxl_fluid_source_set *s, voxl_fluid_grid *g);

// mc rule: two facing full source cells create a new source between them.
// scans the grid and adds any such cells to the set. returns count added.
