...
// sideways neighbour offsets (x,z). no diagonals.
static const int voxl_fluid_dx[4] = { 1, -1, 0, 0 };
static const int voxl_fluid_dz[4] = { 0, 0, 1, -1 };

// can water leave this cell downward into the cell below?
...
int voxl_fluid_water_step(voxl_fluid_grid *g) {
    if (!g) return 0;
    // work into a copy so every cell sees last step's state
    static voxl_fluid_grid voxl_fluid_scratch;
    memcpy(&voxl_fluid_scratch, g, sizeof(*g));

    int changed = 0;
    // top-down so falling water carries through a column in one pass
    ...
    for (int i = 0; i < VOXL_FLUID_GRID_VOL; i++) {
...
int voxl_fluid_water_settle(voxl_fluid_grid *g, int max_steps) {
    ...
    while (steps < max_steps) {
        int changed = voxl_fluid_water_step(g);
        steps++;
        if (changed == 0) break;
    }
    return steps;
