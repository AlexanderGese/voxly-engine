...
int structgen_build_house(const structgen_piece *pc, structgen_buffer *out) {
    ...
    int y0 = fp.y0;
    int wall_h = 3 + structgen_rng_range(&rng, 0, 1);   // 3..4
    int n = 0;
    ...
    // roof: flat plank slab one above the top wall, with a wood-beam border.
    int ry = y0 + wall_h + 1;
    structgen_box roof = structgen_box_make(fp.x0, ry, fp.z0, fp.x1, ry + 1, fp.z1);
    n += structgen_buffer_fill_box(out, roof, BLOCK_WOOD);

    // a torch on the floor by the door so the interior isnt pitch black.
