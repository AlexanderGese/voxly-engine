...
// how fast the player can wind line in, metres/sec, before fight resistance.
#define REEL_BASE_SPEED   3.0f

// tension built per unit of pull while reeling, and how fast it bleeds off when
// you let go. snapping happens when tension reaches max_tension.
#define REEL_TENSION_GAIN 14.0f
#define REEL_TENSION_BLEED 22.0f

// the catch's pull wanders within this band, scaled by its weight.
...
void fishing_reel_step_fight(fishing_reel *rl, fishing_rng *r, float dt) {
    // occasionally pick a new target pull; otherwise drift toward the current
    ...
    // the catch tires the longer the fight runs, capping how hard it can pull.
    rl->stamina -= dt * 0.05f;
