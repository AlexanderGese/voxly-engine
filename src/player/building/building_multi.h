...
// `feet` is the player position for the overlap test. returns #cells applied.
int building_plan_commit(world *w, building_history *hist,
                         const building_plan *plan, vec3 feet);

// apply an erase-plan: set every breakable cell in the plan to air. returns
// #cells cleared. ignores plan->id.
