...
// convenience: pull origin/dir straight off a camera-style eye + forward.
building_target building_target_from_eye(world *w, vec3 eye, vec3 forward,
                                         float reach);

// did we land on something within reach?
int building_target_has_block(const building_target *t);

// distance from eye to the hit block center. used for tie-breaks and fades.
