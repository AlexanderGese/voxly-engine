...
static int pool_acquire(audio3d_pool *p) {
    ...
    // no free slot. steal the one with the lowest current mixed gain. looping
    // sources are slightly protected (they're usually ambience you want kept).
    int   victim = 0;
    float worst  = 1e30f;
    for (int i = 0; i < AUDIO3D_MAX_VOICES; i++) {
...
audio3d_voice *audio3d_pool_voice(audio3d_pool *p, uint32_t handle) {
    ...
    if (p->gen[slot] != AUDIO3D_HANDLE_GEN(handle)) return NULL;  // stale
    audio3d_voice *v = &p->voices[slot];
...
void audio3d_pool_stop(audio3d_pool *p, uint32_t handle, int immediate) {
    ...
    if (immediate) {
        uint32_t slot = AUDIO3D_HANDLE_SLOT(handle);
        v->state = AUDIO3D_VOICE_FREE;
        p->gen[slot]++;
        if (p->active > 0) p->active--;
    } else {
        v->state = AUDIO3D_VOICE_STOPPING;   // mixer ramps fade->0 then reaps
    }
}
...
void audio3d_pool_reap(audio3d_pool *p) {
    ...
    for (int i = 0; i < AUDIO3D_MAX_VOICES; i++) {
        audio3d_voice *v = &p->voices[i];
        if (v->state == AUDIO3D_VOICE_STOPPING && v->fade <= AUDIO3D_EPS) {
            v->state = AUDIO3D_VOICE_FREE;
            p->gen[i]++;
            if (p->active > 0) p->active--;
        }
